...
int insertItem(MemoryList L,void * memAddress,size_t size, char * allocTime, tAlloc typeOfAllocation,
...
void deleteElement(MemoryList L , tAlloc typeOfAllocation,size_t size,char * fileName,unsigned int key){
  int pos;
...
void printShared(MemoryList L){
	...
	{   actualNode = L->nodes[i];
		...
		printf("%18p: size:%zd %s %s (key %d) %s\n", actualNode.memAddress,actualNode.size
