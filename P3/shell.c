...
#include "MemoryList.h"  //this command imports the file 'List.c', which is the location of the list.
#include "HistList.h"
...
/********************* A S S I G M E N T     2   ******************/
    ...
    void Cmd_AsignarMalloc (char * tr[], MemoryList A){
        ...
        size_t size = (size_t) strtoull(tr[0], NULL, 10); //Converts string into integer
...
/***********************************************************************************************/
void CmdFork (){
    //calls a child process, when child terminates, if finishes the execution.
    pid_t chpid;
    if ((chpid = fork()) != -1){
    }else{
...
void ChangePriority (id_t pid, int priority){
...
void CmdPriority (char * tr[]){
    ...
    id_t pid = atoi(tr[0]);
    if (tr[1] == NULL){ 
        //get priority of process "pid"
        int prio = getpriority(PRIO_PROCESS, pid);
        printf("Process: %d, priority: %d\n", pid, prio);
        return;
    }
    //change priority of process 'pid' to 'value'.
    int prio = atoi (tr[1]);
    //priority must be within the gap [-20, 19].
    if (-20 <= prio && prio <= 19){
        ChangePriority(pid, prio);
        //update priority in processes list
        return;
    }
    else {
        perror("Priority not valid. Try a range between [-20, 19]\n");
        return;
    }
    }
    else{ perror("Process not found.\n"); return;}
}

//Function for exec
void generalExec(char * tr[]){
    int priority;
    if(tr[0]!= NULL && tr[0][0] == '@'){
        priority = atoi(tr[0]+1);
        changeTr(tr);
        if (setpriority(PRIO_PROCESS,getpid(),priority)== -1){
            perror("Cannot change the priority");
            return;

           }
    }
    if (execvp(tr[0],tr)){
        perror("Cannot execute the program");
    }
}

void generalplano(char * tr[],int foreground, PList P){
    pid_t pid;
    time_t when;
    time(&when); 
    pid = fork();
    insertP(&P, pid, getpriority(PRIO_PROCESS, pid),
    ctime(&when));
    printf("%d\n",pid );
    if(pid == -1) {
        perror("Cannot create a new process");
...
void CmdPplano(char * tr[], PList P){
    generalplano(tr,1, P);
}

void CmdSplano(char * tr[], PList P){
    generalplano(tr,0, P);
}
...
void CmdProc (char * tr[], PList P){
    if (strcmp(tr[0], "-fg") == 0){
        //info of tr[1] 
    if (tr[1] == NULL) { printPList(P); return; }
    else {
        pid_t pid = (pid_t) atoi(tr[1]);
        //print info of the giving pid
        printPid(P, pid);
        return;
    }
    }
    else{
        perror("Command for 'proc' not found.\n");
        return;
    }
}

void CmdBorrarprocs (char * tr[], PList P){
    if (strcmp(tr[0], "-term") == 0){
        //remove processes status = Terminated_Normally
        deleteElementByState(P, "FINISH");
        return;
    }
    if (strcmp(tr[0], "-sig" ) == 0){
        //remove processes status = Terminated_By_Signal
        deleteElementByState(P, "SIGNAL");
        return;
    }
    else { perror("Command for 'borrarprocs' not found.\n"); return;}
}

void CmdListarProcs(PList P){
    printPList(P);
}
...
struct COMMAND cmd[]={
    ...
    {"proc", CmdProc},
    {"borarprocs", CmdBorrarprocs},
    {"listarprocs",CmdListarProcs},
    {NULL, NULL}
};


void ProcessInput (char * inp, Listh L, MemoryList A, PList P)
{
    char *tr[MAXLINE/2];
    int i;
    int numtr= 0;
    ...
    for (i = 0; cmd[i].name != NULL; i++){
        ...
        if (strcmp(tr[0], cmd[i].name) == 0){   //check the attributes that we give to the functions
            (*cmd[i].pfunc)(tr+1, numtr, A, L, P);    // Once you find the function (tr[0]), you move the position of tr 1 el. -> now the attribute is located at tr[0]
            return;
        }
    }
    if (IsForeground(tr)){
...
{   
    ...
    MemoryList A;
    PList P;
    if(createListH(&L) && createList(&A) && createPList(&P)){
